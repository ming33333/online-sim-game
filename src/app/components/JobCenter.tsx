  ...
  return (
    <Card className="h-full flex flex-col min-h-0">
      <CardHeader className="flex items-center justify-between gap-2 flex-shrink-0 py-2">
        <div>
          <CardTitle className="text-base">Job Center</CardTitle>
          <CardDescription className="text-xs">
            Work during scheduled hours for full pay.
          </CardDescription>
        </div>
      ...
      <CardContent className="flex-1 min-h-0 overflow-y-auto space-y-3 py-2">
        ...
        <div>
          ...
          {jobs.map((job) => {
            ...
            const degreeLabel = job.requiredDegree ? `${job.requiredDegree.charAt(0).toUpperCase()}${job.requiredDegree.slice(1)}` : null;
            return (
            <motion.div
              key={job.id}
